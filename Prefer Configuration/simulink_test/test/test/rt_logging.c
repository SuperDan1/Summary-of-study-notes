...
void* rt_getTempMemory(LogVar* var, const RTWLogSignalInfo* yInfo, int_T i);

void* rt_getTempMemory(LogVar* var, const RTWLogSignalInfo* yInfo, int_T i)
{
    size_t elSize  = var->data.elSize;
    size_t cmplxMult = var->data.complex ? 2 : 1;
    int_T numEls = yInfo->numCols[i];
    void* tempMemory = malloc(elSize * numEls * cmplxMult);
    return tempMemory;
}
        ...
        if (logInfo->y != NULL && updateTXY) {
            ...
            if (ny == 1) {
                ...
                for (i = 0, dataIdx = 0; i < nsig; i++) {
                    RTWPreprocessingFcnPtr preprocessingPtr = yInfo->preprocessingPtrs[i];
                    ...
                    if (preprocessingPtr != NULL) {
                        void* curData = rt_getTempMemory(val, yInfo, i);
                        preprocessingPtr(curData, data[dataIdx]);
                        rt_UpdateLogVar(val,  curData, isVarDims[i]);
                        free( curData );
                    }
                    else {
                ...
                for (i = 0, dataIdx = 0; i < ny && var[i] != NULL; i++) {
                    ...
                    if (preprocessingPtr != NULL) {
                        void* curData = rt_getTempMemory(val, yInfo, i);
                        preprocessingPtr(curData, data[dataIdx]);
                        rt_UpdateLogVar(val,  curData, isVarDims[0]);
                        free( curData );
                    }
                    else {
